...
const output = path.join(root, 'kicad/src/generated/builtin-catalog.json');
const mode = process.argv[2];
if (mode !== 'generate' && mode !== 'check') throw new Error('Expected generate or check');

const result = spawnSync('cargo', ['run', '--quiet', '--manifest-path', 'core/Cargo.toml', '--example', 'export_catalog'], {
  cwd: root,
  encoding: 'utf8',
});
if (result.status !== 0) {
