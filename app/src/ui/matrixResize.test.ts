...
it('extends preset assemblies without copying a key’s local edits or changing existing cells', () => {
  ...
  const matrix = { ...base, rows: 1, columns: 1, definitionId: 'snapshot/switch', cells: [{ row: 0, column: 0, enabled: false, definitionId: 'snapshot/switch', variant: 'preset/mx-rgb/north', rotation: 195, offset: { x: 2, y: 3 }, assembliesLocal: true, assemblies: [{ id: 'led', definitionId: 'snapshot/led', offset: { x: 0, y: -4.75 }, side: 'back' as const }] }] };
...
it('keeps custom sparse matrices on their default behavior', () => {
