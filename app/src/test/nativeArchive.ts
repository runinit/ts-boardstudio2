...
export function nativeArchiveTransport(): ArchiveTransport {
  return {
    async archive(input: NativeRequest) {
      const driver = process.env.BOARDSTUDIO_ARCHIVE_DRIVER ?? join(root, 'core/target/debug/examples/archive_request');
      if (!built && !process.env.BOARDSTUDIO_ARCHIVE_DRIVER) {
        const build = spawnSync('cargo', ['build', '--manifest-path', 'core/Cargo.toml', '--locked', '--example', 'archive_request'], { cwd: root, encoding: 'utf8' });
