import { test, expect } from '@playwright/test';
import { compileSetup, defaultSetup } from '../src/utils/designSetup';
import { CONFIG_LOCAL_STORAGE_KEY } from '../src/context/constants';

const READY_TIMEOUT = 60000;
for (const viewport of [
  { width: 1440, height: 1000 },
  { width: 390, height: 844 },
  { width: 320, height: 740 },
  { width: 844, height: 390 },
]) {
  test(`snapping opens above the view dock and fits at ${viewport.width}px`, async ({
    page,
  }) => {
    test.setTimeout(READY_TIMEOUT);
    await page.setViewportSize(viewport);
    const source = compileSetup({
      ...defaultSetup(),
      columns: 3,
      rows: 3,
      diode: false,
    });
    await page.addInitScript(
      ({ key, source }) => localStorage.setItem(key, JSON.stringify(source)),
      { key: CONFIG_LOCAL_STORAGE_KEY, source }
    );
    await page.route('http://localhost:8400/**', (route) => route.abort());
    await page.goto('./');
    const canvas = page.getByRole('group', {
      name: 'Interactive board layout',
    });
    await expect(canvas).toBeVisible({ timeout: READY_TIMEOUT });
    const tools = page.getByRole('toolbar', {
      name: 'View controls',
      exact: true,
    });
    const snapping = tools.getByRole('button', {
      name: 'Snapping',
      exact: true,
    });
    const arrow = tools.getByRole('button', {
      name: 'Snapping settings',
      exact: true,
    });
    const panel = page.getByRole('region', {
      name: 'Snapping settings',
      exact: true,
    });
    await expect(
      page.getByRole('button', { name: 'Snapping', exact: true })
    ).toHaveCount(1);
    await expect(
      page.getByRole('button', { name: 'Canvas options', exact: true })
    ).toHaveCount(0);
    await expect(arrow).toHaveAttribute('aria-expanded', 'false');
    await expect(panel).toBeHidden();
    await arrow.scrollIntoViewIfNeeded();
    const toolBox = (await snapping.boundingBox())!;
    const arrowBox = (await arrow.boundingBox())!;
    expect(arrowBox.y).toBe(toolBox.y);
    expect(arrowBox.x).toBeGreaterThan(toolBox.x + toolBox.width);
    await expect(arrow).toBeInViewport();
    const closedWidth = (await tools.boundingBox())!.width;
    const closedHeight = (await tools.boundingBox())!.height;
    await arrow.click();
    await expect(panel).toBeVisible();
    await expect(panel).toHaveCSS('position', 'static');
    const dock = (await tools.boundingBox())!;
    expect(dock.width).toBe(closedWidth);
    expect(dock.height).toBe(closedHeight);
    const bounds = (await canvas.locator('..').boundingBox())!;
    const visibleBounds = (await page.getByRole('main').boundingBox())!;
    const expandedPanel = (await panel.boundingBox())!;
    expect(expandedPanel.width).toBeGreaterThanOrEqual(240);
    expect(expandedPanel.height).toBeGreaterThan(0);
    expect(expandedPanel.x).toBeGreaterThanOrEqual(bounds.x);
    expect(expandedPanel.y).toBeGreaterThanOrEqual(bounds.y);
    expect(expandedPanel.y).toBeGreaterThanOrEqual(visibleBounds.y);
    expect(expandedPanel.x + expandedPanel.width).toBeLessThanOrEqual(
      bounds.x + bounds.width
    );
    expect(expandedPanel.y + expandedPanel.height).toBeLessThanOrEqual(dock.y);
    expect(
      await panel.evaluate(
        (element) => element.scrollWidth - element.clientWidth
      )
    ).toBe(0);
    await page.screenshot({
      path: `../.omo/evidence/stitch-ui/snap-${viewport.width}.png`,
    });
    await tools.getByRole('button', { name: 'Footprint origins' }).click();
    await tools.getByRole('button', { name: 'More snapping settings' }).click();
    const custom = panel.getByLabel('Custom snap increment');
    await custom.fill('1.5');
    await custom.press('Escape');
    await expect(panel).toBeHidden();
    await expect(arrow).toBeFocused();
    await snapping.click();
    await expect(snapping).toHaveAttribute('aria-pressed', 'false');
    await arrow.click();
    await tools.getByRole('button', { name: 'More snapping settings' }).click();
    await expect(panel.getByLabel('Custom snap increment')).toHaveValue('1.5');
    await expect(
      tools.getByRole('button', { name: 'Footprint origins' })
    ).toHaveAttribute('aria-pressed', 'true');
    await expect(
      panel.getByRole('button', { name: 'Snap increment 0.25u' })
    ).toBeDisabled();
    await panel
      .getByRole('button', { name: 'Close snapping settings' })
      .click();
    await expect(panel).toBeHidden();
    await expect(arrow).toBeFocused();
  });
}
