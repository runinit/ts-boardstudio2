...
test('split CAD is manual, cancellable, cached and independent of display changes', async ({ page }, info) => {
  ...
  await expect(panel.getByText(/Generation cancelled/)).toBeVisible();
  ...
  await page.getByRole('checkbox', { name: 'Copper', exact: true }).check();
  expect(await measures()).toEqual(before);
  await panel.getByRole('button', { name: 'Generate', exact: true }).click();
  await expect(panel.getByText('Generated geometry is current', { exact: true })).toBeVisible();
  ...
  await expect(panel.getByText(/Generate required/)).toBeVisible();
  await expect(page.getByText(/previous geometry/)).toBeVisible();
  await expect(panel.getByRole('button', { name: 'Export geometry', exact: true })).toBeDisabled();
