import { expect, test, type Page } from "/home/chris/projects/ts-boardstudio2-performance-refactor/app/node_modules/@playwright/test";
import { parse } from "/home/chris/projects/ts-boardstudio2-performance-refactor/app/node_modules/yaml";
import { addCluster, addOutline } from '/home/chris/projects/ts-boardstudio2-performance-refactor/app/src/utils/studioSource';
import { CONFIG_LOCAL_STORAGE_KEY } from '/home/chris/projects/ts-boardstudio2-performance-refactor/app/src/context/constants';
import { openInspector, readSource } from '/home/chris/projects/ts-boardstudio2-performance-refactor/app/e2e/utils/studio';
...
test.beforeEach(async ({ page }) => {
  await page.addInitScript(() => {
    const state = window as Window & { diagnosticPackets: Array<Record<string, unknown>> };
    state.diagnosticPackets = [];
    ...
    window.Worker = class extends OriginalWorker {
      constructor(url: string | URL, options?: WorkerOptions) {
        ...
        this.addEventListener('message', (event) => {
          ...
          if (data && typeof data === 'object') state.diagnosticPackets.push({
            time: performance.now(), type: data.type, stage: data.stage,
            requestId: data.requestId, error: data.error, diagnostics: data.diagnostics,
          });
        });
      }
    };
...
test.afterEach(async ({ page }, info) => {
  ...
  const packets = await page.evaluate(() => (window as Window & { diagnosticPackets: Array<Record<string, unknown>> }).diagnosticPackets);
  await writeFile(info.outputPath('worker-diagnostics.json'), JSON.stringify(packets, null, 2));
  await writeFile(info.outputPath('final-source.yaml'), await readSource(page));
