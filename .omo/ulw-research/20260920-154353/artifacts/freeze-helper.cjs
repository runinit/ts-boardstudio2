...
// src/utils/studioOutline.ts
var studioOutline_exports = {};
__export(studioOutline_exports, {
  freezeOutline: () => freezeOutline,
  freezeOutlines: () => freezeOutlines,
  hasManagedOutline: () => hasManagedOutline,
  isOutlineAutomatic: () => isOutlineAutomatic,
  prepareOutlines: () => prepareOutlines,
  reconcileOutline: () => reconcileOutline,
  setOutlineAutomatic: () => setOutlineAutomatic
});
module.exports = __toCommonJS(studioOutline_exports);

// src/utils/outlineDefaults.ts
var OUTLINE_CLEARANCE = 2;
var OUTLINE_KEY_CLOSE = 2;
var OUTLINE_FILLET = 2;
var OUTLINE_HOLES = "fill";

// src/utils/boardOutlines.ts
var import_layout = require("ergogen/src/native/layout");

// src/utils/boardTopology.ts
var import_layout2 = require("ergogen/src/native/layout");

// src/utils/setupRepair.ts
var import_yaml2 = __toESM(require_dist());

// src/utils/designSetup.ts
...
// src/utils/resizeReview.ts
var import_yaml3 = __toESM(require_dist());

// src/utils/studioSource.ts
var import_yaml8 = __toESM(require_dist());

// src/utils/sourceSnapshot.ts
...
function snapshot(source) {
  ...
  const document2 = (0, import_yaml4.parseDocument)(source, { keepSourceTokens: true });
  if (document2.errors.length) {
    throw new Error(document2.errors[0].message);
  }
  cached = { source, document: document2, value: document2.toJS() };
  return cached;
}
function sourceDocument(source) {
  return snapshot(source).document.clone();
}
function sourceValue(source, path = []) {
  const value = path.reduce(
    (value2, key) => value2 && typeof value2 === "object" ? value2[key] : void 0,
    snapshot(source).value
  );
  return structuredClone(value);
}

// src/utils/removeSourceValues.ts
var import_yaml5 = __toESM(require_dist());

// src/utils/designSource.ts
...
function editDesign(source, path, value) {
  const doc = document(source);
  const node = doc.getIn(path, true);
  if ((0, import_yaml6.isScalar)(node) && node.range) {
    return source.slice(0, node.range[0]) + renderValue(source, node.range[1], true, value) + source.slice(node.range[1]);
  }
  if (node !== void 0) {
    throw new Error(
  ...
  while (depth > 0 && doc.getIn(path.slice(0, depth), true) === void 0) {
    depth--;
  }
  const parent = depth ? doc.getIn(path.slice(0, depth), true) : doc.contents;
  if (!(0, import_yaml6.isMap)(parent) || !parent.range) {
    throw new Error(
...
function appendMapping(source, parent, addition) {
  if (!parent.range) {
    throw new Error("This field has no editable source range.");
  }
  if (parent.flow) {
    const rendered2 = (0, import_yaml6.stringify)(addition, {
      collectionStyle: "flow",
      aliasDuplicateObjects: false,
      lineWidth: 0
    }).trim();
    const end2 = source.lastIndexOf("}", parent.range[1] - 1);
    return source.slice(0, end2) + (parent.items.length ? ", " : "") + rendered2.slice(1, -1).trim() + source.slice(end2);
  }
  const first = parent.items[0]?.key;
  const start = (0, import_yaml6.isScalar)(first) && first.range ? first.range[0] : parent.range[0];
  const indent = start - source.lastIndexOf("\n", start - 1) - 1;
  const rendered = (0, import_yaml6.stringify)(addition, {
    lineWidth: 0,
    aliasDuplicateObjects: false
  }).trimEnd().split("\n").map((line) => " ".repeat(indent) + line).join("\n");
  const end = parent.range[2];
  const newline = source.includes("\r\n") ? "\r\n" : "\n";
  return source.slice(0, end) + (source[end - 1] === "\n" ? "" : newline) + rendered.replace(/\n/g, newline) + newline + source.slice(end);
}
function editField(source, path, value) {
  const doc = sourceDocument(source);
  if (doc.errors.length) {
    throw new Error(doc.errors[0].message);
  }
  const node = doc.getIn(path, true);
  if (!node || (0, import_yaml6.isScalar)(node)) {
    return editDesign(source, path, value);
  }
  if (!(0, import_yaml6.isNode)(node) || !node.range) {
    throw new Error("This field has no editable source range.");
  }
  return source.slice(0, node.range[0]) + renderValue(source, node.range[1], false, value) + source.slice(node.range[1]);
}
function renderValue(source, end, scalar, value) {
  const next = (0, import_yaml6.stringify)(value, {
    collectionStyle: "flow",
    lineWidth: 0,
    aliasDuplicateObjects: false,
    flowCollectionPadding: !scalar
  }).trimEnd();
  const newline = !scalar && source[end - 1] === "\n" ? source.includes("\r\n") ? "\r\n" : "\n" : "";
  return next + newline;
}

// src/utils/layoutSource.ts
var import_yaml7 = __toESM(require_dist());

// src/utils/designUnits.ts
var import_assert = require("ergogen/src/assert");

// src/utils/studioSource.ts
function readStudio(source) {
  const data = sourceValue(source);
...
function removeValue(source, path) {
  const doc = sourceDocument(source);
  if (doc.errors.length) {
    throw new Error(doc.errors[0].message);
  }
  if (doc.getIn(path, true) === void 0) {
    return source;
  }
  const parent = doc.getIn(path.slice(0, -1), true);
  if (!(0, import_yaml8.isMap)(parent)) {
    throw new Error("Remove a named field from a mapping.");
  }
  const index = parent.items.findIndex(
    (item) => (0, import_yaml8.isScalar)(item.key) && item.key.value === path.at(-1)
  );
  if (index < 0) {
    return source;
  }
  const entry = parent.items[index];
  if (!(0, import_yaml8.isScalar)(entry.key) || !entry.key.range || !(0, import_yaml8.isNode)(entry.value) || !entry.value.range) {
    throw new Error("This field has no editable source range.");
  }
  let start = entry.key.range[0], end = entry.value.range[2];
  if (parent.items.length === 1) {
    return editField(source, path.slice(0, -1), {});
  }
  if (parent.flow) {
    end = entry.value.range[1];
    const next = parent.items[index + 1]?.key;
    if ((0, import_yaml8.isScalar)(next) && next.range) {
      end = next.range[0];
    } else {
  ...
  const check = (0, import_yaml8.parseDocument)(result);
...
function nextId(ids, base) {
  const prefix = base.replace(/[^A-Za-z_0-9-]/g, "_").replace(/^[^A-Za-z_]/, "_$&") || "item";
...
function addOutline(source, board = "main", report, mode = "create") {
  const data = readStudio(source);
  const selected = data.pcbs?.[board]?.profile;
  const previousProfile = mode === "replace" && selected?.startsWith("profiles.") ? selected.slice("profiles.".length) : "";
  const previousProfileSpec = data.designs?.profiles?.[previousProfile];
  const previousFrom = previousProfileSpec?.from;
  const previousBoundary = typeof previousFrom === "string" && previousFrom.startsWith("boundaries.") ? previousFrom.slice("boundaries.".length) : "";
  ...
  for (const [id, item] of Object.entries(objects)) {
    if (item.kind === "anchor" || item.pcb && item.pcb !== board || !item.pcb && item.kind !== "key") {
      continue;
    }
    const envelopes = {
    ...
    const name = item.kind === "key" && envelopes.keycap ? "keycap" : envelopes.pcb ? "pcb" : envelopes.body ? "body" : "";
    if (name) {
  ...
  let result = source;
  const sources = [];
  const previousRegionRefs = Array.isArray(previous.from) ? previous.from.filter((item) => typeof item === "string").filter((item) => item.startsWith("regions.")).map((item) => item.slice("regions.".length)) : typeof previous.from === "string" && previous.from.startsWith("regions.") ? [previous.from.slice("regions.".length)] : [];
  const otherUses = {
    ...data,
    designs: {
      ...data.designs,
      boundaries: {
        ...
        [previousBoundary]: { ...previous, from: void 0 }
      }
    }
  };
  const ownershipPath = ["meta", "studio", "outline", "managed", board];
  const references = (value, ref, path = []) => {
    if (path.length === ownershipPath.length && path.every((key, index) => key === ownershipPath[index])) {
      return false;
    }
    if (value === ref) {
      return true;
    ...
    return Object.entries(value).some(
      ([key, child]) => references(child, ref, [...path, key])
    );
  };
  const previousRegions = previousRegionRefs.filter((id) => {
    const candidate = data.designs?.regions?.[id];
    const select = candidate?.select;
    return candidate && select?.pcb === board && Object.keys(select).every((key) => ["kind", "pcb"].includes(key)) && Object.keys(candidate).every(
      (key) => [
        "select",
        "envelope",
        "close",
  ...
  const reusedRegions = /* @__PURE__ */ new Set();
  for (const group of Object.keys(groups)) {
    const [kind, envelope] = group.split(":");
    const current = readStudio(result);
    const region = previousRegions.find((id) => {
      if (reusedRegions.has(id)) {
        return false;
      }
      const candidate = current.designs?.regions?.[id];
      return candidate?.envelope === envelope && candidate.select?.kind === kind;
    }) || nextId(
      Object.keys(current.designs?.regions || {}),
      `${board}_${envelope}`
    );
    reusedRegions.add(region);
    const recipe = { ...current.designs?.regions?.[region] || {} };
    if (kind !== "key" && recipe.close === OUTLINE_KEY_CLOSE) {
      delete recipe.close;
    }
    result = editField(result, ["designs", "regions", region], {
      ...recipe,
      select: Object.keys(groups).filter((key) => key.startsWith(`${kind}:`)).length === 1 ? { kind, pcb: board } : { ids: groups[group] },
      envelope,
      ...kind === "key" ? {
        close: current.designs?.regions?.[region]?.close ?? OUTLINE_KEY_CLOSE
      } : {}
    });
    sources.push(`regions.${region}`);
  }
  const profile = previousProfile || nextId(Object.keys(data.designs?.profiles || {}), `${board}_outline`);
  const physical = Object.values(groups).flat();
  const linked = new Set(physical.slice(0, 1));
  const bridges = {};
  const position = (id) => report?.objects[id]?.position || [0, 0, 0];
  while (linked.size < physical.length) {
    const pairs = Array.from(linked).flatMap(
      (a2) => physical.filter((b2) => !linked.has(b2)).map((b2) => ({
        a: a2,
        b: b2,
        distance: Math.hypot(
          ...position(a2).map((v, i) => v - position(b2)[i])
        )
      }))
    );
    pairs.sort((a2, b2) => a2.distance - b2.distance);
    const pair = pairs[0];
    const a = objects[pair.a], b = objects[pair.b];
    if ((!a.cluster || a.cluster !== b.cluster) && (!report || pair.distance > 1e-3)) {
  ...
  const boundary = previousBoundary || nextId(Object.keys(data.designs?.boundaries || {}), `${board}_edge`);
  const previousBridges = previous.bridges || {};
  ...
  result = editField(result, ["designs", "boundaries", boundary], {
    clearance: OUTLINE_CLEARANCE,
    simplify: 2,
    corners: { fillet: OUTLINE_FILLET },
    holes: getValue(source, ["designs", "profiles", previousProfile, "holes"]) ?? OUTLINE_HOLES,
    connected: "single",
    ...previous,
    from: sources,
    bridges: {
  ...
  result = editField(result, ["designs", "profiles", profile], {
    ...data.designs?.profiles?.[profile],
    from: `boundaries.${boundary}`
  });
  result = editField(result, ["pcbs", board, "profile"], `profiles.${profile}`);
  if (!data.pcbs?.[board]?.thickness) {
    result = editField(result, ["pcbs", board, "thickness"], 1.6);
  }
  for (const id of Object.keys(data.layout.objects || {})) {
    if (data.layout.objects[id].kind === "key" && !data.layout.objects[id].pcb) {
      result = editField(result, ["layout", "objects", id, "pcb"], board);
    }
  }
  return result;
}
...
function modelSnapshot(model) {
  ...
  const visit = (value, parent) => {
    ...
    const point = (p) => [p[0] + offset[0], p[1] + offset[1]];
...
function managed(source, board) {
  ...
  const boundary = data.designs?.boundaries?.[boundaryId];
  ...
  if (owned?.includes(profile) && owned.includes(from)) {
    const regions = Array.isArray(boundary?.from) ? boundary.from : [boundary?.from];
    return [
      ...regions.filter((ref) => typeof ref === "string"),
      from,
      profile
    ];
  }
  const legacy = boundary;
  const knownRecipe = legacy?.clearance === OUTLINE_CLEARANCE && legacy.connected === "single" && legacy.corners?.fillet === OUTLINE_FILLET && Object.keys(profileSpec || {}).every(
    (key) => ["from", "snapshot"].includes(key)
  );
  if (!knownRecipe && (!(profileId === board || profileId.startsWith(`${board}_outline`)) || !(boundaryId === board || boundaryId.startsWith(`${board}_edge`)))) {
    return [];
  }
  const refs = Array.isArray(boundary?.from) ? boundary.from : [boundary?.from];
  if (!refs.length || !refs.every((ref) => {
    if (typeof ref !== "string" || !ref.startsWith("regions.")) {
      return false;
    }
    const region = data.designs?.regions?.[ref.slice("regions.".length)];
    return region?.select?.pcb === board && !!region.select.kind && !!region.envelope;
  })) {
    return [];
  }
  return [...refs, from, profile];
}
function hasManagedOutline(source) {
  try {
    const data = readStudio(source);
    return Object.keys(data.pcbs || {}).some(
      (board) => !data.pcbs?.[board]?.profile || managed(source, board).length > 0
    );
  } catch {
    return false;
  }
}
function isOutlineAutomatic(source) {
  try {
...
function setOutlineAutomatic(source, auto) {
  return editField(source, ["meta", "studio", "outline", "auto"], auto);
}
function freezeOutline(source, board, snapshot2) {
  const boundary = managed(source, board).find(
    (ref) => ref.startsWith("boundaries.")
  );
...
function freezeOutlines(source, report) {
  let result = source;
...
function prepareOutlines(source, report) {
  let result = source;
  for (const board of Object.keys(readStudio(source).pcbs || {})) {
...
function reconcileOutline(source, _board = "main", report) {
