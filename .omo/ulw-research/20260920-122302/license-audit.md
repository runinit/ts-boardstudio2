...
4. KiSwitch upstream describes the library as dual MIT/CC-BY-SA; local `manifest/kiswitch.json` and the bundled license preserve only the CC-BY-SA branch. The local metadata records only the CC-BY-SA branch and omits the upstream README’s dual-license statement.
