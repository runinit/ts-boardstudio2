...
import { matrixCellId, matrixMembers, cellPose, splayOriginWorld, withSplayOrigin, withSplayAngle } from './matrixGeometry';
...
const Workbench = ({ document, scene, onEdit, onUndo, onRedo, onExport, compileFootprints, onNewProject, onImport, onImportFootprint, onImportModel, onSelectLibraryModel, libraryModelPreviews, libraryModelStatus, onRequestCaseModels, onDuplicateDesign, onModeChange, casePreview, componentPreviews, embedUsedModels = true, onEmbedUsedModelsChange, selectedBoardId: selectedBoardIdProp, onSelectBoard }: Props) => {
  ...
  const [splayPreview, setSplayPreview] = useState<Matrix | null>(null);
  const splayDrag = useRef<{ matrix: Matrix; pending: Matrix; column: number; kind: 'origin' | 'angle'; pointerId: number; startAngle: number; target: SVGGElement; transactionId: string; bounds: ReturnType<typeof getBounds> } | null>(null);
    ...
    const handleKey = (event: KeyboardEvent) => {
      ...
      if (event.key === 'Escape') {
        if (splayDrag.current) {
          ...
          splayDrag.current = null; setSplayPreview(null); return;
        }
  ...
  const treeEntries = useMemo<TreeEntry[]>(() => {
    ...
    return result;
  }, [mode, selected, document.caseBodies, activeCaseBody?.id, selectedBoardId, selectedBoard?.name, visibleMatrices, treeVisibleParts, expandedTree, scope, treeParts, definitions, matrixCellOverrides, selectScope, toggleTree, treeGrouping, memberMaps, document.matrices, boardLayouts]);
  ...
  const drawOutline = (event: React.MouseEvent<SVGSVGElement>) => {
    ...
    if (originPicking && scope?.matrixId && scope.column !== undefined) {
      ...
      if (matrix && point) commitMatrix(withSplayOrigin(matrix, scope.column, event.altKey ? point : snapDelta(point, matrix.pitch, snapFraction)));
  ...
  const startSplay = (event: React.PointerEvent<SVGGElement>, kind: 'origin' | 'angle') => {
    ...
    event.currentTarget.setPointerCapture(event.pointerId);
    const origin = splayOriginWorld(matrix, scope.column);
    splayDrag.current = { matrix, pending: matrix, column: scope.column, kind, pointerId: event.pointerId,
  ...
  const moveCanvasPointer = (event: React.PointerEvent<SVGSVGElement>) => {
    ...
    if (splay && splay.pointerId === event.pointerId) {
      const point = pointFromEvent(event, svgRef.current);
      if (!point) return;
      if (splay.kind === 'origin') splay.pending = withSplayOrigin(splay.matrix, splay.column, event.altKey ? point : snapDelta(point, splay.matrix.pitch, snapFraction));
      else {
        const origin = splayOriginWorld(splay.matrix, splay.column);
        ...
        splay.pending = withSplayAngle(splay.matrix, splay.column, event.altKey ? value : Math.round(value), splayAffect);
      }
      setSplayPreview(splay.pending);
      commitMatrix(splay.pending, 'preview', splay.transactionId);
  ...
  const endCanvasPointer = (event: React.PointerEvent<SVGSVGElement>) => {
    const splay = splayDrag.current;
    if (splay && splay.pointerId === event.pointerId) {
      if (splay.pending !== splay.matrix) commitMatrix(event.type === 'pointercancel' ? splay.matrix : splay.pending, event.type === 'pointercancel' ? 'preview' : 'commit', splay.transactionId);
      if (splay.target.hasPointerCapture(splay.pointerId)) splay.target.releasePointerCapture(splay.pointerId);
      splayDrag.current = null; setSplayPreview(null);
      suppressClick.current = true;
      window.setTimeout(() => { suppressClick.current = false; }, 0);
      return;
    }
  ...
  const selectedMatrix = splayPreview ?? (scope?.matrixId ? matrixMap.get(scope.matrixId) : undefined);
  ...
  const getModeDetails = () => {
    ...
    if (selectedMatrix && scope && scope.kind !== 'component') return <>
      ...
      {scope.kind === 'matrix'
        ...
        : <CellInspector splayAffect={splayAffect} onAffectChange={setSplayAffect} onPickOrigin={() => { setOriginPicking(true); setRightOpen(false); }} matrix={selectedMatrix} scope={scope} definitions={libraryDefinitions} parts={treeParts} members={memberMaps.get(selectedMatrix.id) ?? new Map()} onChange={(matrix, definitions) => commitMatrix(matrix, 'commit', undefined, definitions)} />}
        ...
        <div className={`wb-canvas-stage ${leftOpen || rightOpen ? 'has-drawer' : ''} ${mode === 'Design' && !showFootprints ? 'is-layout-simplified' : ''}`}>
          ...
          <svg tabIndex={0} onKeyDown={(event) => { if (matrixGhost) {
            ...
            <g transform="scale(1,-1)" style={outlineActive || pendingPart || originPicking ? { pointerEvents: 'none' } : undefined}>
              ...
              {mode === 'Design' && pairPreview && <><line className="wb-mirror-axis" x1={matrixGhost!.origin.x} x2={matrixGhost!.origin.x} y1={matrixGhost!.origin.y - 16} y2={matrixGhost!.origin.y + (matrixGhost!.rows - 1) * matrixGhost!.pitch.y + 16} /><MatrixGhost matrix={pairPreview.preview} cells={new Map()} scope={null} ghost onSelect={() => undefined} onStagger={() => undefined} /></>}
              {mode === 'Design' && matrixGhost && <MatrixGhost matrix={pairPreview?.matrix ?? matrixGhost} cells={new Map()} scope={null} ghost onSelect={() => undefined} onStagger={() => undefined} />}
              ...
              {mode === 'Design' && selectedMatrix && scope?.kind === 'column' && !originPicking && <SplayHandles matrix={selectedMatrix} column={scope.column ?? 0} parts={activeParts} onStart={startSplay}
                onOrigin={(point) => commitMatrix(withSplayOrigin(selectedMatrix, scope.column ?? 0, point))}
                onAngle={(value) => commitMatrix(withSplayAngle(selectedMatrix, scope.column ?? 0, value, splayAffect))} />}
...
const CellInspector = ({ matrix, scope, definitions, parts, members, onChange, splayAffect, onAffectChange, onPickOrigin }: {
  ...
  if (scope.kind === 'row' || scope.kind === 'column') {
    ...
    return <section aria-label={`${axis} properties`}>
      {axis === 'column' && <InspectorSection title="Splay & origin" defaultOpen>
        <div className="wb-splay-angle"><CaseNumber label="Splay" value={matrix.columnSplays?.[index] ?? 0} unit="°" validation="finite" onCommit={(value) => onChange(withSplayAngle(matrix, index, value, splayAffect))} /></div>
        <label className="wb-script-select-label">Origin<select aria-label="Splay origin" value={matrix.columnOrigins?.[index] ? 'custom' : 'base'} onChange={(event) => onChange(withSplayOrigin(matrix, index, event.target.value === 'base' ? null : splayOriginWorld(matrix, index)))}><option value="base">Column base</option><option value="custom">Custom point</option></select></label>
        <button className="wb-secondary wb-pick-origin" onClick={onPickOrigin}><ToolIcon name="origin" />Pick origin</button>
        <div className="wb-matrix-number-grid">{field('Origin X', splayOriginWorld(matrix, index).x, (x) => onChange(withSplayOrigin(matrix, index, { ...splayOriginWorld(matrix, index), x })))}{field('Origin Y', splayOriginWorld(matrix, index).y, (y) => onChange(withSplayOrigin(matrix, index, { ...splayOriginWorld(matrix, index), y })))}</div>
...
const SplayHandles = ({ matrix, column, parts, onStart, onOrigin, onAngle }: {
  matrix: Matrix; column: number; parts: Part[];
  onStart: (event: React.PointerEvent<SVGGElement>, kind: 'origin' | 'angle') => void;
  onOrigin: (point: Vec2) => void; onAngle: (angle: number) => void;
}) => {
  const origin = splayOriginWorld(matrix, column);
  ...
  return <g className="wb-splay-handles" onClick={(event) => event.stopPropagation()}>
    ...
    <g transform={`translate(${x} ${y})`} role="button" tabIndex={0} aria-label="Drag to splay" onPointerDown={(event) => onStart(event, 'angle')} onKeyDown={(event) => {
      ...
      event.preventDefault(); event.stopPropagation(); onAngle((matrix.columnSplays?.[column] ?? 0) + (event.key === 'ArrowRight' ? 1 : -1) * (event.shiftKey ? 5 : 1));
    }}>
