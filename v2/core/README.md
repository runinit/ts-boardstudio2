...
`wasm-pack build v2/core --target web --release` emits the worker package in
`v2/core/pkg`. `CoreEngine.request(json)` accepts and returns the JSON protocol
from `v2/contracts`. Preview replies contain projected scene geometry while
...
Native outline timings: `cargo test --release --manifest-path v2/core/Cargo.toml
...
--manifest-path v2/core/Cargo.toml --lib -- --ignored --nocapture`.
